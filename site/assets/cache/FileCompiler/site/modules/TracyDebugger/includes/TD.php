...
class TD extends TracyDebugger {
    ...
    private static function generateDump($var, $options) {
        ...
        if(count(\TracyDebugger::getDataValue('dumpPanelTabs')) > 0 && !is_string($var)) {
            ...
            $out .= '</ul>';
            if(($var instanceof \ProcessWire\Wire || $var instanceof \ProcessWire\Wire) && $var->id)   {
    ...
    private static function generateEditLink($var, $type, $section) {
        return '<span style="float:right"><a href="' . \ProcessWire\wire('config')->urls->admin . $section . ($section ? '/' : '') . $type . '/edit/?id=' . $var->id . '" title="Edit ' . trim($type, 's') . ': ' . $var->name . '">#' . $var->id . '</a></span>';
    }
